...
void triBulles(String[][] tableau, int colonne) {
    int borneMax = tableau.length - 1;
    boolean permutation = true;
    while (permutation) {
        permutation = false;
        for (int i = 0; i < borneMax; i++) {
            if (colonne == 3) {
                String[] prix1 = tableau[i][colonne].split("\\$");
                String[] prix2 = tableau[i+1][colonne].split("\\$");
                double p1 = Double.parseDouble(prix1[0].replaceAll("[^\\d]", ""));
                double p2 = Double.parseDouble(prix2[0].replaceAll("[^\\d]", ""));
                if (p1 > p2) {
                ...
                int annee1 = Integer.parseInt(tableau[i][colonne]);
                int annee2 = Integer.parseInt(tableau[i+1][colonne]);
                if (annee1 > annee2) {
                    String[] temp = tableau[i];
                    tableau[i] = tableau[i+1];
                    tableau[i+1] = temp;
    ...
    String triPageParValeur(String[][] tab, int colonne) {
        String res = "";
        String htmlp = "";
        String[] headers = {"NOM", "DATE", "PRIX", "ENTREPRISE", "DESCRIPTION"};
        String[] links = {"produits-nom.html", "produits-date.html", "produits-prix.html", "produits-entreprise.html", "produits-description.html"};

        // Sort the table based on the selected column
        String[][] colonneTriee = tab;
        triBulles(colonneTriee, colonne);
        for (int i = 0; i < tab.length; i++) {
            for (int j = 0; j < tab.length; j++) {
                if (colonneTriee[i].equals(tab[j])) {
                    String[] temp = tab[i];
                    tab[i] = tab[j];
                    tab[j] = temp;
                }
        ...
        for (int i = 0; i < headers.length; i++) {
                res += "<a href=\"" + links[i] + "\">" + headers[i] + "</a>";
        ...
        for(int i = 0; i<length(html); i++) {
            ...
            if(charAt(html, i) == '>') {
                if(equals(substring(html, i-2, i+1), "<p>")) {
                    htmlp += "\n" + res;
                } else if (equals(substring(html, i-4, i+1), "</li>")) {
                    htmlp += "\n<li><a href=\"produits-nom.html\">Produits</a></li>";
                }
        }
            if(charAt(html, i) == '<') {
                if(equals(substring(html, i, i+4), "<h2>")) {
                    htmlp += "h2>Liste de l'ensemble des ordinateurs";
                    i += 3;
                }
            }
        }
...
String[][] chargerProduits(String repertoire, String prefixe) {
    ...
    void algorithm() {
        ...
        String triNom = triPageParValeur(tabProduit, 0);
        String triDate = triPageParValeur(tabProduit, 1);
        String triEntreprise = triPageParValeur(tabProduit, 2);
        String triPrix = triPageParValeur(tabProduit, 3);
        String triDescription = triPageParValeur(tabProduit, 4);

        // On crée les fichiers triés
        stringAsFile("./output/produits-nom.html", triNom);
        stringAsFile("./output/produits-date.html", triDate);
        stringAsFile("./output/produits-entreprise.html", triEntreprise);
        stringAsFile("./output/produits-prix.html", triPrix);
        stringAsFile("./output/produits-description.html", triDescription);
    }
}